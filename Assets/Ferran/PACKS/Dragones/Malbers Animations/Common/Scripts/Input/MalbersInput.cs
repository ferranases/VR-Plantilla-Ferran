...
namespace MalbersAnimations
{
    public class MalbersInput : MonoBehaviour, IInputSource
    {
        ...
        void InitializeCharacter()
        {
            ...
            if (mCharacter != null)
            {
                Dictionary<string, BoolEvent> keys = new Dictionary<string, BoolEvent>();
        ...
        protected virtual void SetInput()
        {
            ...
            //{
            //    inputs[8].OnInputChanged.Invoke(true);
            //}

            foreach (InputRow item in inputs) { bool InputValue = item.GetInput; }

            //This will set the Current Input value to the inputs and Invoke the Values
        }
        public void volar()
        {
            inputs[10].OnInputChanged.Invoke(true);
        }

        public void subirAltura()
        {
            inputs[14].OnInputChanged.Invoke(true);
        }

        public void bajarAltura()
        {
            inputs[13].OnInputChanged.Invoke(true);
        }

        public void velocidadDragon(float velocidad, int animacion)
        {
            this.velocidad = velocidad;
            if (this.animacion != animacion)
            {
                inputs[(4 + animacion)].OnInputChanged.Invoke(true);
